    ...
    <Card className="border-green-200 bg-green-50/30 overflow-hidden shadow-xl animate-in fade-in slide-in-from-bottom-4 duration-700">
      <CardHeader className="bg-green-100/50 border-b border-green-100">
        ...
        <CardDescription className="text-green-700 font-medium flex items-center gap-2 mt-1">
          <Languages className="h-3 w-3"/> {data.disease} ({data.hindi_name} / {data.marathi_name})
        </CardDescription>
      </CardHeader>
      ...
      <CardContent className="pt-6 space-y-8">
        ...
        <div className="p-4 bg-orange-50/50 rounded-xl border border-orange-100 space-y-3">
          <div className="flex items-center gap-2 text-orange-800">
            ...
            <h4 className="text-xs font-bold uppercase tracking-widest">Medical Context & Complications</h4>
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-[11px]">
            <div>
              <p className="font-bold text-orange-700 mb-1">Modern Interventions:</p>
              <p className="text-muted-foreground">{data.medical_intervention.join(', ')}</p>
            </div>
            <div>
              <p className="font-bold text-orange-700 mb-1">Potential Risks:</p>
              <p className="text-muted-foreground">{data.complications.join(', ')}</p>
            </div>
          </div>
        </div>
        ...
        <div className="flex flex-col items-center gap-2 pt-4 border-t border-green-100">
          ...
          <p className="italic text-[10px] text-muted-foreground text-center">
            "AyurGenixAI recommendations are for informational purposes. Consult a qualified practitioner or medical doctor before following any advice. Prevention: {data.prevention}."
          </p>
        </div>
      </CardContent>
    </Card>
