...
import { Coins, Trophy, History, ShieldCheck, Heart, Hospital, FileText, ArrowUpRight, Zap, ExternalLink } from "lucide-react";
...
export default function RewardsPage() {
  ...
  useEffect(() => {
    ...
    if (currentUser) {
      getRewards(currentUser.uid).then(data => {
        setRewards(data);
        setIsLoading(false);
      });
    } else {
  ...
  const milestoneProgress = (rewards.points / nextMilestone) * 100;
          ...
          <CardContent className="space-y-6">
            ...
            <div className="grid grid-cols-2 gap-4 pt-4">
              <div className="bg-background/50 p-4 rounded-lg border border-primary/10">
                <p className="text-xs text-muted-foreground uppercase font-bold">Wallet Address</p>
                <p className="text-sm font-mono truncate">{user.walletAddress || '0x...Connecting'}</p>
              </div>
              <div className="bg-background/50 p-4 rounded-lg border border-primary/10">
                <p className="text-xs text-muted-foreground uppercase font-bold">Trust Network</p>
                <p className="text-sm font-semibold flex items-center gap-1">Avalanche Fuji <Zap className="h-3 w-3 text-yellow-500 fill-yellow-500"/></p>
              </div>
            </div>
          </CardContent>
        </Card>
        ...
        <Card className="shadow-lg border-accent/20">
          ...
          <CardContent className="space-y-4">
            <div className="flex items-center justify-between p-2 rounded hover:bg-muted/50 transition-colors">
              <div className="flex items-center gap-3">
                <div className="p-2 bg-red-100 rounded-full"><Heart className="h-4 w-4 text-red-600"/></div>
            ...
            <div className="flex items-center justify-between p-2 rounded hover:bg-muted/50 transition-colors">
              <div className="flex items-center gap-3">
                <div className="p-2 bg-blue-100 rounded-full"><Hospital className="h-4 w-4 text-blue-600"/></div>
            ...
            <div className="flex items-center justify-between p-2 rounded hover:bg-muted/50 transition-colors">
              <div className="flex items-center gap-3">
                <div className="p-2 bg-green-100 rounded-full"><FileText className="h-4 w-4 text-green-600"/></div>
          ...
          <CardFooter>
            <Button asChild variant="outline" className="w-full">
              <Link href="/records/upload">Earn Points Now</Link>
            </Button>
          </CardFooter>
        </Card>
      </div>
                ...
                <div className="flex items-center gap-6">
                  ...
                  {activity.txHash && (
                    <Button variant="ghost" size="icon" asChild>
                      <Link href={`https://testnet.snowtrace.io/tx/${activity.txHash}`} target="_blank">
